...
2. **Controller Contract** (Controller)
   ...
   - Redemption queue functions: `priceBatch` (settles at live `eveBasePriceInETH()` before marking this batch priced), `processRequest`, `processRequests` (in-window only; expired batches revert `ExitQueueBatchExpired`)
