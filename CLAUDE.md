...
11. **CRE Keeper Receivers** (CREQueueExecutor, CREStrategyExecutor)
   ...
   - **CREQueueExecutor** (`QueueAction`: PriceBatch, ProcessRequests, AdvanceCursor):
     - `queueUpkeepStatus()`: gas-bounded fallback/cross-check (`MAX_BATCH_SCAN = 25`) — ProcessRequests / PriceBatch / AdvanceCursor priority matching the historical keeper design
     ...
     - Cursor / affordability / `minBatchAge` / `maxUsersPerUpkeep` semantics unchanged from the prior CLA design
   - **CREStrategyExecutor** (`StrategyAction`, priority: Rebalance, WithdrawShortfall, ProvideExitLiquidity, DepositExcess, HarvestPerformanceFees, Sync):
     ...
     - Pending redemption needs anchored at `CREQueueExecutor.nextLiveBatchIdToProcess()`
