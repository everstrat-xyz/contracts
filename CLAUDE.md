...
- `src/`: Solidity smart contracts
  ...
  - `contracts/automation/KeeperExecutorBase.sol`: Shared base for Gelato keeper executors (caller allowlist, pause, reentrancy guard)
  - `contracts/automation/QueueKeeperExecutor.sol`: Gelato keeper for the redemption queue (on-chain checker + TS Web3 Function)
  - `contracts/automation/StrategyKeeperExecutor.sol`: Gelato keeper for strategy operations (on-chain checker / Solidity Function)
...
11. **Gelato Keeper Executors** (QueueKeeperExecutor, StrategyKeeperExecutor)
   - Static (non-upgradeable) Gelato keepers under `src/contracts/automation/`, sharing `KeeperExecutorBase` (RegistryClient + OZ Pausable + ReentrancyGuard)
   - **Trust model**: `Gelato task (dedicated msg.sender proxy) → Keeper*Executor → Controller`. Executors are the ONLY automated `KEEPER_ROLE` holders; Gelato infra never receives a protocol role. `perform` is callable only by an ADMIN-managed executor-caller allowlist (`allowExecutorCaller` / `removeExecutorCaller`, `EnumerableSet`); an empty allowlist makes the executor inert (`KeeperExecutorNoAllowedCallers`). Populated with the task's dedicated msg.sender **after** task creation
   - **Gelato surfaces**: W2 (`StrategyKeeperExecutor`) is a Solidity Function — Gelato polls `checker()`, which mirrors `strategyUpkeepStatus()` and returns exact `perform` calldata. W1 (`QueueKeeperExecutor`) also has a TS Web3 Function (deep queue scan, `keepers/` repo) that emits the same `perform` calldata
