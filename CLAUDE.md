...
- `src/`: Solidity smart contracts
  ...
  - `contracts/automation/CREReceiverBase.sol`: Shared base for Chainlink CRE / Keystone report receivers
  - `contracts/automation/CREQueueExecutor.sol`: CRE receiver for the redemption queue
  - `contracts/automation/CREStrategyExecutor.sol`: CRE receiver for strategy operations
  - `interfaces/`: Contract interfaces
    ...
    - `automation/IReceiver.sol`: CRE / Keystone `onReport` consumer interface
    - `automation/ICREReceiverBase.sol`: Shared CRE receiver interface (forwarder, workflow binding)
    - `automation/ICREQueueExecutor.sol`: CREQueueExecutor interface
    - `automation/ICREStrategyExecutor.sol`: CREStrategyExecutor interface
...
| `KEEPER_ROLE` / `MINTER_ROLE` | CREQueueExecutor + CREStrategyExecutor / AMM (+ StrategyManager for fee mint) | none | Operational roles, granted through the 48h timelock after finalize (or by deployer bootstrap before finalize). |
...
11. **CRE Keeper Receivers** (CREQueueExecutor, CREStrategyExecutor)
   - Static (non-upgradeable) Chainlink CRE / Keystone receivers under `src/contracts/automation/`, sharing `CREReceiverBase` (RegistryClient + OZ Pausable + ReentrancyGuard + `IReceiver` / ERC-165)
   - **Trust model**: `CRE DON → KeystoneForwarder → CRE*Executor → Controller`. Executors are the ONLY automated `KEEPER_ROLE` holders; Chainlink infra never receives a protocol role. `onReport` is callable only by the immutable constructor `FORWARDER`. Workflow identity is ADMIN-bound (`expectedWorkflowOwner` / `expectedWorkflowName` / `expectedWorkflowId`); unbound receivers reject reports
   ...
   - **CREStrategyExecutor** (`StrategyAction`, priority: Rebalance, WithdrawShortfall, ProvideExitLiquidity, DepositExcess, HarvestPerformanceFees, Sync):
     - `strategyUpkeepStatus()` exposes the recommended action + estimated amount; `_processReport` recomputes amounts and never trusts the report for ETH quantities. `StrategyUpkeepPerformed` emits the Controller return (ETH actually moved / harvest `feeETHEquivalent`), which may be less than the estimate on SM try/catch underfill. A 0 actual is emitted as a successful no-op — it does not revert `NoUpkeepNeeded` (that would roll back the whole report). ProvideExitLiquidity has no return (`sendValue` is all-or-nothing); the event uses the recomputed `topUp`.
   ...
   - Tests: `test/unit/CREQueueExecutor.t.sol`, `test/unit/CREStrategyExecutor.t.sol`; trees: `test/trees/CREReceiverBase.tree` (shared `onReport` auth / Envelope branches), `test/trees/CREQueueExecutor.tree`, `test/trees/CREStrategyExecutor.tree`; fork metadata guard: `test/fork/CREKeystoneMetadata.t.sol`
   - Deployment: `script/DeployCREExecutors.s.sol` / `DeployAll` via `ProtocolDeployBase._deployCREExecutors` (requires `KEYSTONE_FORWARDER`, `CHAIN_SELECTOR`, `MAX_REPORT_AGE`, plus policy knobs). Register under `QUEUE_KEEPER_EXECUTOR` / `STRATEGY_KEEPER_EXECUTOR`, grant `KEEPER_ROLE`, then bind workflow identity and enable `writeReport`. The two executors are the only `KEEPER_ROLE` holders a deployment creates; a manual break-glass keeper multisig is an opt-in, separately timelocked governance decision documented in `docs/FREEZE_RUNBOOK.md` §0.1 (rationale, risk surface, containment, signer policy) — set `strategyDepositCooldown` > 0 before granting keeper roles to new addresses.
