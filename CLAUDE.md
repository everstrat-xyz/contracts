...
- `src/`: Solidity smart contracts
  ...
  - `contracts/automation/KeeperExecutorBase.sol`: Shared base for Mimic-driven keeper executors (caller allowlist, pause, reentrancy guard)
  - `contracts/automation/QueueKeeperExecutor.sol`: Keeper for the redemption queue (on-chain checker + Mimic deep-scan function)
  - `contracts/automation/StrategyKeeperExecutor.sol`: Keeper for strategy operations (on-chain checker, relayed by a thin Mimic function)
  - `interfaces/`: Contract interfaces
    ...
    - `automation/IKeeperExecutorBase.sol`: Shared keeper executor interface (caller allowlist, pause)
...
|------|--------|-------|-------|
| `SECURITY_ROLE` | security multisig | none | Emergency response (no timelock): `pause()` on AMM, Controller, ExitQueue, StrategyManager, UniCLStrat, Converter, and Registry (a Registry pause also freezes role grants), plus the emergency capital-recovery paths `Controller.emergencyExitToAMM()` (sweep idle Controller ETH to the AMM), `StrategyManager.emergencyWithdrawToController()` (sweep idle StrategyManager ETH back to the Controller), and `UniCLStrat.emergencyExit()` (sweep a paused strategy's held funds back to StrategyManager; the pool unwind itself happens best-effort at `pause()` time), plus `StrategyManager.removeSupportedERC20()` (instant stale-feed / dust-NAV unfreeze; `addSupportedERC20` stays ADMIN-only), plus `Whitelist.removeSigner()` (instant revoke of a leaked invite-signing key; `addSigner` stays ADMIN-only). Cannot unpause, configure, or upgrade. |
...
- **Deployment** (`script/DeployAll.s.sol` / `ProtocolDeployBase`): the 48h admin timelock is deployed first, the Registry is constructed with it as its designated admin (the constructor rejects `_admin == msg.sender`, so the deployer key cannot be the designated admin), the deployer bootstraps wiring (including the initial Oracle feed, Whitelist, and both keeper executors) with its temporary Registry ADMIN grant and renounces everything at the end. The modular flow mirrors this: `DeployRegistry` deploys the admin timelock + Registry (timelock as designated admin, DAO as proposer); later steps read `TIMELOCK_ADDRESS`; `DeployWhitelist` registers `WHITELIST` and may seed `WHITELIST_SIGNER_ADDRESS`; `DeployKeeperExecutors` registers both executors under `QUEUE_KEEPER_EXECUTOR` / `STRATEGY_KEEPER_EXECUTOR`, optionally grants them `KEEPER_ROLE` (`GRANT_KEEPER_ROLE`), and applies `EXIT_LIQUIDITY_TARGET_ETH` / `CONTROLLER_RESERVE_ETH` (required wei env; `0` is a valid explicit choice). Executors deploy **inert**: after the Mimic triggers are created, ADMIN calls `allowExecutorCaller(smartAccount)` on each (post-finalize this is 48h-timelocked). Env (required, never defaulted to the deployer): `DAO_ADDRESS` (non-zero — sole timelock proposer; `address(0)` is rejected), `SECURITY_ADDRESS`, `DAO_TREASURY_ADDRESS`, `EXIT_LIQUIDITY_TARGET_ETH`, `CONTROLLER_RESERVE_ETH`, `WHITELIST_SIGNER_ADDRESS` (explicit `address(0)` postpones invite-signer seeding); optional numeric: `TIMELOCK_ADMIN_DELAY` (default 48h; values below 48h revert), `PERFORMANCE_FEE_BPS` (default 0).
...
2. **Controller Contract** (Controller)
   ...
   - Keeper functions: `depositToStrategies`, `depositToStrategy`, `withdrawFromStrategies`, `withdrawFromStrategy`, `checkAndRebalanceStrategies`, `checkAndRebalanceStrategy`, `syncStrategies`, `syncStrategy`, `provideExitLiquidity`, `harvestPerformanceFeeFromStrategy`, `harvestPerformanceFeeFromStrategies`. Deposit/withdraw/harvest return the StrategyManager actual (ETH deposited/withdrawn, or `(eveAmount, feeETHEquivalent)`); the executor emits those on `StrategyUpkeepPerformed`. `provideExitLiquidity` does not return — `sendValue` is all-or-nothing, so the executor emits the recomputed `topUp`.
   - Redemption queue functions: `priceBatch` (settles at live `eveBasePriceInETH()` before marking this batch priced), `processRequest`, `processRequests` (in-window only; expired batches revert `ExitQueueBatchExpired`). The full-batch `processRequests(batchId)` is a no-op when nothing remains (already settled); the ranged overload still reverts `ExitQueueInvalidRange` on an empty `[start, end)`.
...
3. **ExitQueue Contract** (ExitQueue)
   ...
   - **`MAX_LIVE_PRICED_BATCHES` = 25:** `priceBatch` reverts `ExitQueueTooManyLivePricedBatches` if the live-scan width would exceed this. Keeper `MAX_BATCH_SCAN` is this value (both alias `ExitQueueLimits.MAX_LIVE_PRICED_BATCHES`). **Not a production cadence target** — keeper `minBatchAge` (1 day) vs the 3-day window implies ~3 overlapping batches. The cap is an `enter()` gas / DoS bound.
  ...
  - AMM functions: pushRequest(), pullRequest(), closeRequest() returns (bool _viaEscapeHatch) — push/pull gated by `whenNotPaused`; closeRequest works when paused. `pushRequest` reverts `ExitQueueTokensOverflow` if `totalTokensToBurn` would exceed `uint128`.
  - Controller functions: priceBatch() — gated by `whenNotPaused`
  - View functions: batchInfo(), requestInfo(), requestCanBeClosed(), unprocessedUsersCount(), unprocessedUsers(), liveRedemptionOffsets(), liveScanFromBatchId(), MAX_BATCH_PROCESSING_TIME(), MAX_LIVE_PRICED_BATCHES()
...
5. **AMM Contract** (AMM)
   ...
   - **Split Pricing (inception spec)**: `enter()` (mint) prices at `premium_price = NAV/(supply·cw)` and `exit()` (burn) prices at `base_price = NAV/supply` (formulas in `Math.premiumPrice`/`Math.basePrice`). **Live NAV / live supply:** both formulas use StrategyManager NAV after deducting in-window priced ExitQueue liability, and EVE `totalSupply` minus in-window escrowed tokens (`AMM._liveSupply`). Unpriced queued EVE stays in the denominator (cancellable equity). Reverts `AMMEscrowExceedsSupply` if escrow exceeds supply. Minting pays the bonding-curve premium; burning redeems at the NAV-backed base price so EVE is always redeemed against its backing and the base price grows with supply as the retained premium accrues to remaining holders. `Controller.priceBatch()` settles queued batches at `eveBasePriceInETH()` **before** marking this batch priced (this batch is still equity; already-priced in-window batches are already deducted). Views: `eveBasePriceInETH()`, `evePremiumPriceInETH()`, `eveBasePriceInUSD()`, `evePremiumPriceInUSD()`.
   ...
   - Errors: AMMNotWhitelisted (enter while invite gate active and caller not whitelisted), AMMNoClaimableBalance (thrown by claim() when claimableBalances[msg.sender] == 0), AMMTooLowBatchExitETH (queued exit below minBatchExitETH), AMMInvalidMinBatchExitETH (setMinBatchExitETH above MIN_BATCH_EXIT_ETH_UPPER_BOUND), AMMEscrowExceedsSupply (in-window priced escrow exceeds EVE totalSupply)
   - Version: 1.0.0
...
6. **StrategyManager Contract** (StrategyManager)
   ...
   - **ETH-First NAV Calculation**: NAV in ETH first, USD via Oracle when needed. Total NAV = sum of `strategy.navInETH()` (any revert freezes protocol) + StrategyManager ETH balance + Controller ETH balance + AMM `freeBalance()` (excludes `lockedForClaims`) + ETH value of each whitelisted supported-ERC-20 balance (via `Oracle.convert(token, address(0), balance, IERC20Metadata.decimals(), 18)`) **minus in-window priced ExitQueue liability** (`liveRedemptionOffsets().liabilityETH`). Unpriced queued EVE is still equity and is not deducted. Liability lapses when a batch exceeds `ExitQueue.MAX_BATCH_PROCESSING_TIME` (view-only; no reset tx). Reverts `StrategyManagerQueuedLiabilityExceedsNAV` if liability exceeds gross NAV (fail-closed). **`_totalNAVInETH()` reverts if any strategy's `navInETH()` reverts** — prevents minting EVE at a discount on under-reported NAV
...
8. **UniCLStrat Contract** (UniCLStrat)
   ...
   - Deployment: `script/DeployUniCLStrat.s.sol` (env includes `UNIV3_FACTORY` + `POOL_ADDRESS`; constructor verifies `factory.getPool(token0, token1, pool.fee()) == pool`)
   ...
   - Pool trust: constructor requires WETH in the pool pair **and** Uniswap V3 factory provenance (`factory.getPool(token0, token1, fee) == pool`) so a malicious interface-compatible pool cannot be the trusted `uniswapV3MintCallback` caller.
   - Key Functions:
     ...
     - `pause()`: `ADMIN_ROLE` or `SECURITY_ROLE`. Flips the pause flag first (local state only, so a degraded pool can never block the circuit breaker), then attempts the pool unwind as a best-effort try/catch self-call (`selfRemoveLiquidityAndCollect()` — poke-then-accrue then remove/collect; a pool revert emits `LiquidityUnwindSkipped` instead of bubbling up), then revokes the WETH Converter allowance strictly and best-effort-revokes the paired token via try/catch self-call (`selfRevokePairedTokenConverterAllowance`; a paused/blacklisted token emits `ConverterAllowanceRevocationSkipped` and does not roll back the pause, the unwind, or the WETH revoke). Leftover paired-token approvals while paused are inert (`whenNotPaused` on every Converter-calling path). `unpause()` (`ADMIN_ROLE` only) restores Converter allowances strictly (fail-closed).
     - `emergencyExit()`: `ADMIN_ROLE` or `SECURITY_ROLE` (not StrategyManager). Requires pause. Transfers only what the strategy already holds — it never touches the pool: unwraps WETH strictly via `weth.withdraw()` and sends native ETH to `strategyManager` first; best-effort recovers `pairedToken` (try/catch on `balanceOf`, then `trySafeTransfer` — emits `PairedTokenTransferSkipped` on either failure; retry with a later `emergencyExit()`); writes off pending LP fees via `_resetLpFeeAccounting` (best-effort `_tryAccrueLpFees` with aggregate snapshot fallback if either `positions` read reverts, then `charged = earned`); emits `EmergencyExited(ethAmount)`. The paired token stays visible in NAV when whitelisted via `StrategyManager.addSupportedERC20()` (on-chain swap recovery via the shared Converter is deferred to a follow-up PR). If the pause-time pool unwind was skipped (degraded pool), the LP position stays in the pool and remains attributed to the strategy via `navInETH()`; once the pool functions again the admin recovers it via `unpause()` followed by normal withdrawals or a re-run of `pause()` (which performs the unwind) + `emergencyExit()`.
   ...
   - Calm-Period Guards: `_isCalm()` checks spot tick and short TWAP against long TWAP ± maxTickDeviation. The invariant is **never mint into a dislocated pool** — not "never touch the pool when dislocated." `deposit()` / `investIdleETH()` / `rebalance()` revert `UniCLStratNotCalm`; `withdraw()` burns and converts unconditionally (NAV is TWAP/oracle-marked; conversion is independently bounded by quote/oracle/slippage) and only gates the re-add. Rationale and math: `docs/STRATEGY_GUARDRAILS.md` §1.1.1 and `mermaid/uniswap-concentrated-liquidity-strategy-spec.md` (Calm-Period Gate Asymmetry).
   - TWAP Window Floors: `MIN_TWAP_INTERVAL = 1800` (30 min) and `MIN_SHORT_TWAP_INTERVAL = 60` are lower bounds; `MAX_TWAP_INTERVAL = uint16.max × MAX_BLOCK_SECONDS` (~9.1 days) is the upper bound (a longer window cannot be densely served by Uniswap's uint16 observation ring). Enforced in the constructor and the `setTwapInterval()`/`setShortTwapInterval()` setters (`UniCLStratInvalidConfig`). Constructor and both setters also require (1) `pool.observe([interval, 0])` (`UniCLStratPoolTWAPNotAvailable`) so NAV will compute, and (2) in-use `observationCardinality >= ceil(interval / MAX_BLOCK_SECONDS)` floored at `MIN_OBSERVATION_CARDINALITY = 150` (`UniCLStratInsufficientObservationCardinality`) so a quiet cardinality-1 pool cannot mark at a spot-extrapolated TWAP. The long TWAP anchors both the calm band and the LP-position composition in `navInETH()` (which feeds StrategyManager NAV and AMM pricing), so biasing it requires sustaining a skewed tick across ~150 blocks; mint sizing in `_liquidityForPosition()` uses spot (`slot0`) because Uniswap charges mint callbacks at the current price. The short floor matches the UniswapV3ConverterAdapter's `MIN_TWAP_INTERVAL` so a single-block flash-loan skew cannot pass the calm check. Runtime `navInETH()` still fail-closes if a later `observe` reverts. `DeployUniCLStrat.s.sol` repeats both probes before broadcast.
   ...
   - Events: FundsDeposited, FundsInvested, FundsWithdrawn, Rebalanced, Synced, EmergencyExited, LiquidityUnwindSkipped (best-effort pool unwind on pause reverted and was skipped), ConverterAllowanceRevocationSkipped (best-effort paired-token Converter `approve(0)` on pause reverted and was skipped), PairedTokenTransferSkipped (best-effort paired-token transfer on emergencyExit reverted and was skipped)
   - Errors: UniCLStratZeroAddress, UniCLStratInvalidPool, UniCLStratInvalidConfig, UniCLStratNotCalm, UniCLStratNotPaused, UniCLStratInsufficientWETH, UniCLStratCallerNotPool, UniCLStratCallerNotSelf, UniCLStratInvalidMintCallback, UniCLStratPoolTWAPNotAvailable, UniCLStratInsufficientObservationCardinality
...
9. **Converter Contract** (Converter)
 ...
 - **Adapter Allowlist**: Adapters must be explicitly whitelisted by ADMIN_ROLE via `setAllowedAdapter()`. Only whitelisted adapters can be used for swaps and quotes. SECURITY: whitelisted adapters run with full access to the Converter's storage and balances during swaps — only audited, stateless adapters may be whitelisted.
   - **Role-Based Access Control**:
     - `ADMIN_ROLE`: Self-administered. Can configure adapters, unpause, authorize upgrades. Can also pause (shared with SECURITY).
     - `SECURITY_ROLE`: Can `pause()` instantly (circuit breaker). Cannot unpause, configure adapters, or upgrade.
 ...
 - `quoteSwapExactAmountOut(address, bytes, uint256)`: On-chain quote via adapter (exact-output, regular CALL). Same non-view rationale as `quoteSwapExactAmountIn`. Permissionless.
   - **Pausable**: Can be paused by `ADMIN_ROLE` or `SECURITY_ROLE` (all wrap/unwrap/swap operations revert when paused); `unpause` is `ADMIN_ROLE` only.
   - **Key Principles**:
...
11. **Mimic Keeper Executors** (QueueKeeperExecutor, StrategyKeeperExecutor)
   - Static (non-upgradeable) keepers under `src/contracts/automation/`, sharing `KeeperExecutorBase` (RegistryClient + OZ Pausable + ReentrancyGuard), driven by Mimic automation
   - **Trust model**: `Mimic smart account → Keeper*Executor → Controller`. Executors are the ONLY automated `KEEPER_ROLE` holders; Mimic infra never receives a protocol role. `perform` is callable only by an ADMIN-managed executor-caller allowlist (`allowExecutorCaller` / `removeExecutorCaller`, `EnumerableSet`); an empty allowlist makes the executor inert (`KeeperExecutorNoAllowedCallers`). Populated with the operator's smart account **after** the Mimic triggers are created
   - **Mimic surfaces**: W2 (`StrategyKeeperExecutor`) has a thin Mimic function that reads `checker()` via oracle and relays the exact `perform` calldata. W1 (`QueueKeeperExecutor`) has a Mimic function performing the deep queue scan (keepers repo, `mimic-functions/queue-keeper`) that emits the same `perform` calldata
   - **Payload is untrusted for amounts**: it carries the action + hints only; conditions and amounts are re-validated/recomputed from live state in `_processReport`, reverting with `KeeperExecutorNoUpkeepNeeded` on stale/wrong claims
   ...
   - **QueueKeeperExecutor** (`QueueAction`: PriceBatch, ProcessRequests, AdvanceCursor):
     - `queueUpkeepStatus()` / `checker()`: gas-bounded on-chain scan (`MAX_BATCH_SCAN` aliased from `ExitQueueLimits.MAX_LIVE_PRICED_BATCHES`; a DoS bound, not cadence — production overlap is ~3) — ProcessRequests / PriceBatch / AdvanceCursor priority
     - ProcessRequests params: `abi.encode(batchId, startIndex, endIndex)` (exclusive end); must be an affordable prefix from index 0
     - After `MAX_BATCH_PROCESSING_TIME`, `_affordableRequests` returns 0 (expired batches are skippable; `pullRequest` would revert `ExitQueueBatchExpired`). **Do not use the keeper cursor for NAV** — `advanceBatchCursor` can skip live batches; share-price offsets walk `ExitQueue.liveScanFromBatchId`
   - **StrategyKeeperExecutor** (`StrategyAction`, priority: Rebalance, WithdrawShortfall, ProvideExitLiquidity, DepositExcess, HarvestPerformanceFees, Sync):
     - `strategyUpkeepStatus()` exposes the recommended action + estimated amount; `_processReport` recomputes amounts and never trusts the payload for ETH quantities. `StrategyUpkeepPerformed` emits the Controller return (ETH actually moved / harvest `feeETHEquivalent`), which may be less than the estimate on SM try/catch underfill. A 0 actual is emitted as a successful no-op — it does not revert `NoUpkeepNeeded` (that would roll back the whole execution). ProvideExitLiquidity has no return (`sendValue` is all-or-nothing); the event uses the recomputed `topUp`.
     - `_depositCapacityAvailable` requires `!cooldown && isHealthy() && maxDeposit() > 0 && depositWeight > 0` so all-zero deposit weights cannot recommend DepositExcess
     - Pending redemption needs anchored at `QueueKeeperExecutor.nextLiveBatchIdToProcess()` (priced in-window batches at `finalEvePrice`; expired contribute 0). The current **unpriced** batch is not counted — still cancellable equity, matching `liveRedemptionOffsets`; `WithdrawShortfall` waits until `priceBatch` so a queue-then-cancel cannot pull LP
   ...
   - Tests: `test/unit/QueueKeeperExecutor.t.sol`, `test/unit/StrategyKeeperExecutor.t.sol`; trees: `test/trees/KeeperExecutorBase.tree` (shared allowlist/pause branches), `test/trees/QueueKeeperExecutor.tree`, `test/trees/StrategyKeeperExecutor.tree`
   - Deployment: `script/DeployKeeperExecutors.s.sol` / `DeployAll` via `ProtocolDeployBase._deployKeeperExecutors` (requires `EXIT_LIQUIDITY_TARGET_ETH`, `CONTROLLER_RESERVE_ETH`, `GRANT_KEEPER_ROLE`, plus policy knobs). Register under `QUEUE_KEEPER_EXECUTOR` / `STRATEGY_KEEPER_EXECUTOR`, optionally grant `KEEPER_ROLE`, then deploy the Mimic functions, create their triggers, and call `allowExecutorCaller(smartAccount)` on each executor. The two executors are the only `KEEPER_ROLE` holders a deployment creates; a manual break-glass keeper multisig is an opt-in, separately timelocked governance decision documented in `docs/FREEZE_RUNBOOK.md` §0.1 (rationale, risk surface, containment, signer policy) — set `strategyDepositCooldown` > 0 before granting keeper roles to new addresses.
...
13. **Supporting Libraries**
   ...
   - `ExitQueueLimits.sol`: Shared `MAX_LIVE_PRICED_BATCHES` (25) aliased by ExitQueue and both keeper `MAX_BATCH_SCAN` constants
...
- **UniCLStrat**: Fully tested static strategy contract
  ...
  - Features: Uniswap V3 concentrated liquidity, TWAP-based NAV composition with spot-based mint sizing, calm-period guards on mints (withdraw unwind ungated — STRATEGY_GUARDRAILS §1.1.1), emergency exit. Performance fees use strategy-local LP-fee accounting with StrategyManager dilution settlement
  - Interface: `src/interfaces/strategies/IUniCLStrat.sol`
